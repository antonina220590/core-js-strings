...
function isPalindrome(str) {
  const lowerStr = str.toLowerCase().replace(/[\W_]/g, '');
...
function findLongestWord(sentence) {
  ...
  return splited.reduce((x, y) => (y.length > x.length ? y : x));
}
...
function reverseWords(str) {
  ...
  return splited.split(' ').reverse().join(' ');
}
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, (x) =>
    String.fromCharCode(x.charCodeAt() + (x.toLowerCase() < 'n' ? 13 : -13))
  );
}
